// patch_weight_calc_button.cjs
// Serbest Cizim butonunun ALTINA, bagimsiz yeni bir "Agirlik Hesapla"
// butonu ve kendi ekranini (src/weightcalc.jsx) ekler. Malzeme + olcu
// (uzunluk x genislik x kalinlik x adet) girilince ozgul agirliga gore
// net agirlik (kg) hesaplar. Malzemeler: DKP, Galvaniz, INOX 304/316,
// Aluminyum 1050/5754, Hardox.
...
const WEIGHTCALC_JSX = path.join(ROOT, "src", "weightcalc.jsx");
...
/* ---------------------------------------------------------------------
   1) Yeni dosya: src/weightcalc.jsx
--------------------------------------------------------------------- */

const WEIGHTCALC_CONTENT = `import React, { useMemo, useState } from "react";
...
fs.writeFileSync(WEIGHTCALC_JSX, WEIGHTCALC_CONTENT, "utf8");
console.log("[OK] src/weightcalc.jsx olusturuldu");
...
replaceOnce(
  ...
  "Agirlik Hesapla butonu ve ekrani eklendi"
);

console.log("\n✅ AGIRLIK HESAPLA BUTONU BASARIYLA EKLENDI.");
console.log("Simdi: git add -A && git commit -m \"Yeni bagimsiz Agirlik Hesapla butonu eklendi\" && git push -u origin main");
