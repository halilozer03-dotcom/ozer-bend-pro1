...
export default function FreeDrawCanvas({ maxSegments, onCommit, onClose }) {
  ...
  // Yatay/dikey kilidi YOK - telefonun o anki yonune gore ekran boyutunu
  // olcup viewBox'i dogru oranla ayarliyoruz (bosluk/kayma olmasin diye).
  useEffect(() => {
  ...
  const finish = () => {
    ...
    const finalSegments = segs.map((s, i) => {
      ...
      while (diff < -180) diff += 360;
      // NOT: Ekran koordinatlarinda Y asagi dogru artar, uygulamanin ic
      // geometri motoru (computeGeneralPoints) matematiksel (Y yukari)
      // yon kullaniyor - bu yuzden buku yonu isareti burada TERS cevriliyor,
      // boylece cizdigin sekil 3D/Genel Profil'de AYNI yonde cikar.
  ...
  const halfW = halfH * (containerAspect || 1);
  const vbX = -halfW;
  const vbY = -halfH;
  const vbW = halfW * 2;
  const vbH = halfH * 2;
  ...
  if (drag && anchorPoint) {
    ...
    const labelW = Math.min(vbW * 0.5, viewSize * 0.32);
  ...
  return (
    <div ref={wrapRef} style={{ position: "fixed", inset: 0, background: "#05070a", zIndex: 999 }}>
      ...
      >
        {gridLines}
        <line x1={vbX} y1={0} x2={vbX + vbW} y2={0} stroke="rgba(255,255,255,0.18)" strokeWidth={viewSize / 250} />
      ...
      {/* Yuzen rozet: segment sayaci - her zaman ust-sol, yon degismez */}
      <div style={{
      ...
      {/* Yuzen kapat butonu - her zaman ust-sag */}
      <button
        onClick={onClose}
        style={{
      ...
      {/* Alt toolbar - guvenli alan boslugu ile, her zaman en altta */}
      <div style={{
