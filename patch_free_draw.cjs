// patch_free_draw.cjs
// 1) Yeni src/freedraw.jsx dosyasini olusturur: dokunmatik cizim, 5mm izgara
//    yapisma, 15 derece aci kilidi, cizim SIRASINDA canli "___ mm / ___°"
//    etiketi, max 15 segment siniri.
// 2) main.jsx icine "Serbest Cizim" butonunu ve entegrasyonunu ekler.
//    Cizilen sekil -> mevcut segments[] formatina cevrilip Genel Profil
//    moduna otomatik gecirilir (BD/PDF sistemine tam bagli).
...
if (fs.existsSync(FREEDRAW_JSX)) {
  console.log("[BILGI] src/freedraw.jsx zaten var, ustune yazilacak.");
}
fs.writeFileSync(FREEDRAW_JSX, FREEDRAW_CONTENT, "utf8");
console.log("[OK] src/freedraw.jsx olusturuldu");

/* ---------------------------------------------------------------------
   2) main.jsx entegrasyonu
--------------------------------------------------------------------- */
...
replaceOnce(
  ...
  "freeDrawOpen state eklendi"
);

/* 2c) Buton + kosullu render */
replaceOnce(
  MAIN_JSX,
  ...
  "Serbest Cizim butonu ve overlay entegrasyonu eklendi"
);

console.log("\n✅ SERBEST CIZIM OZELLIGI (FAZ 1) BASARIYLA EKLENDI.");
console.log("NOT: Buton simdilik sadece Turkce metin gosteriyor, diger dillere ceviri sonra eklenebilir.");
console.log("NOT2: Cizilen sekildeki buku yonu (dir) isareti ters gelirse (sekil aynali cikarsa) bunu birlikte kalibre ederiz.");
console.log("Simdi: git add -A && git commit -m \"Serbest Cizim ozelligi (Faz 1) eklendi\" && git push -u origin main");
